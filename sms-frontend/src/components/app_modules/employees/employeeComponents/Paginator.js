...
export const Paginator = ({ staffData, setTableData, tableData }) => {
  ...
  const nextButtonClicked = (e) => {
    ...
    if (newPage === lastPage) {
      if (employeeCount % tableLength > 0) {
        console.log(employeeCount % tableLength);
        setTableData(staffData.slice(-((employeeCount % tableLength) + 1), -1));
  ...
  const lastButtonClicked = (e) => {
    ...
    if (lastPage === firstPage) {
      return;
    } else if (employeeCount % tableLength > 0) {
      console.log(employeeCount % tableLength);
      setTableData(staffData.slice(-((employeeCount % tableLength) + 1), -1));
  ...
  useEffect(() => {
    if (currentPage === 1) {
      setEmployeeCount(staffData.length);
